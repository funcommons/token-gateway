...
When LLM-face routing or pre-deduction fails, an original capability-face code that hits the passthrough whitelist (`gateway.error-passthrough-codes`) is returned to you as-is with a semantic HTTP status. Default whitelist: 4090→403 (risk-control rejection), 10601→402 (insufficient balance), 10602→404, 10603→404, 10402→409 (state conflict), 10612→403 (IP whitelist rejection); misses still follow the legacy gateway mapping (see [Conventions §3](./conventions.md)).
