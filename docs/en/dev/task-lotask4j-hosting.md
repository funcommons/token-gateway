...
| Alternative form | For private-deployment/no-platform scenarios, keep the self-owned DB delegation surface as an alternative (the TaskClient SPI contract is frozen but **reserved and unimplemented** — no implementation exists in the repo today, so switching requires a separately-scoped project first; it is an adapter decision, not an architectural fork). **retask4j evaluation verdict**: a Redis queue framework with no durable task table/query/admin plane — unsuitable for state hosting; only a candidate execution-dispatch layer in the lightweight private-deployment combo (self-owned DB + retask4j dispatch) |
