...
| **M2.5 Task face merged in** | lotask4j **zero-modification onboarding** (V4+ platform prerequisite: tenant isolation RLS + webhook HMAC built in; R2/R6/R7 compensated gateway-side; only R5 manual state change remains an optional increment) + face-task (caller endpoints + billing saga + signature-verifying notify receiver + resource proxy + LotaskTaskClient + timeout-clock/reconciliation fallback) + self-written Worker (Groovy three hooks, script source of truth in this repo's `scripts/`) | Smoke: four-modality create→poll→resource proxy works end-to-end; face=task group independently deployed and verified; task billing (full pre-consume / terminal refund) reconciles with zero discrepancy; webhook three-header verification + verify-then-act requery fallback + degradation matrix drills |
