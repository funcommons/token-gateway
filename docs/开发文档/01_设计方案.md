...
| 形态 | 路由 | 状态机位置 | 适用 |
|---|---|---|---|
| **本地状态机**（默认） | route 面 resolve → 上游 base_url + 出站凭证 | 网关 face-task（THMP 移植）：先路由定价再全额预扣 → 出站 → 轮询状态机（SUCCEEDED/FAILED/EXPIRED）→ notify（HMAC 签名 + 退避重发）→ 资源代理（sig 能力凭证，上游 URL 永不透传） | 上游是"哑"任务 API（create/poll），网关统一任务体验与终态保障 |
...
**任务面管线**（face=task，本地状态机形态；**控制层决策 · 数据面执行**）：create（**控制层 key 验证 → 控制层路由表 resolve（先路由定价：模型不同价格不同）→ 按命中模型计量并全额预扣 → 出站 POST tasks**）→ 调用方轮询（3~5s 驱动，终态幂等不触上游）→ notify 回调（HMAC + 退避重发）→ 资源代理（GET 签名 URL，流式回源 + 本地缓存）。复用 route/billing/moderation/log/audit 面；调度兜底 = 孤儿预扣释放 / 过期扫描 / notify 重发（THMP MaintenanceScheduler 同款）。
