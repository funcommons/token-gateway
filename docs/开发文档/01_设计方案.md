...
token-gateway:
  ...
  token-validate:                        # 凭证校验服务
    url: http://localhost:9400
    auth: token
    token: ${GW_ROUTE_TOKEN}             # 同一服务可复用同一凭证
...
| type | 发送方式 | 适用 |
|---|---|---|
| `none` | 不发鉴权头 | 内网信任 / 白名单网络 |
| `key` | 静态 key 头 `X-API-Key: <key>` | 简单服务间共享密钥 |
| `jwt` | HS256 签名 JWT（`Authorization: Bearer`，claims 含 iss/caller/tenant_id）——**现网 internal-token 即此形态**（后端换 secret 须网关同步重铸） | 需要身份语义（调用方/租户）的服务间调用 |
| `token` | 静态不透明令牌头 `X-Internal-Token: <token>` | 既有静态令牌兼容 |

- 凭证一律环境变量注入禁入仓；日志/管理面只出现掩码。
- TokenHub 契约面 HMAC 四头（X-Access-Key/Timestamp/Nonce/Signature）不走此四式——它是 **adapter=tokenhub 的协议形状**（§6），配置在 `route:` 面。
...
| 适配器 | 能力实现来源 | 协议形状 | 说明 |
|---|---|---|---|
| `MmagixAdapter` | 现 `rpc/*` 7 客户端原样平移（HttpTokenApi/HttpChannelApi/HttpBillingApi/HttpModerationApi/HttpAccessLogApi/HttpChatModelApi），日志/审计调用点拆到对应能力面 | §5.2 四式（jwt=现网 internal-token） | 七能力全量；M1 行为零变化 |
...
| `openapi`（通用） | **内置**，按 《能力面接口契约》调用任意后端——**三方不限语言**（Go/Python/Node 实现 HTTP 端点或 MQ 消费即可接入） | 《能力面接口契约》（信封 + 鉴权四式） | 三方接入首选；无需写 Java |
...
| 形态 | 路由 | 状态机位置 | 适用 |
|---|---|---|---|
| **本地状态机**（默认） | route 面 resolve → 上游 base_url + 出站凭证 | 网关 face-task（THMP 移植）：创建即全额预扣 → 出站 → 轮询状态机（SUCCEEDED/FAILED/EXPIRED）→ notify（HMAC 签名 + 退避重发）→ 资源代理（sig 能力凭证，上游 URL 永不透传） | 上游是"哑"任务 API（create/poll），网关统一任务体验与终态保障 |
...
**任务面管线**（face=task，本地状态机形态）：create（计量 → 计费全额预扣 → route resolve → 出站 POST tasks）→ 调用方轮询（3~5s 驱动，终态幂等不触上游）→ notify 回调（HMAC + 退避重发）→ 资源代理（GET 签名 URL，流式回源 + 本地缓存）。复用 route/billing/moderation/log/audit 面；调度兜底 = 孤儿预扣释放 / 过期扫描 / notify 重发（THMP MaintenanceScheduler 同款）。
...
| **THMP 任务域**（TaskService/轮询状态机/ResourceService/ResourceSigner/NotifySender/MaintenanceScheduler） | 平移进 **face-task**（88% 覆盖的已验证实现，不新写）；17 号 DDL 任务表 + 分区随行 |
...
token-gateway/
  ...
  face-task          # 任务面：四模态 create/poll/notify/资源代理 + 调度兜底（THMP 任务域移植）
...
| **M2.5 任务面并入** | face-task（THMP 任务域移植：状态机/notify/资源代理/调度兜底 + 任务表迁移）+ 调用方四模态端点 + task 委托面 | 冒烟：四模态 create→poll→资源代理走通；face=task 分组独立部署验证；task 计费（全额预扣/终态退款）对账零差异 |
...
| 10 | 任务域移植面较大（状态机/资源缓存/调度兜底 + 任务表迁移） | 中 | 全部为 THMP 已验证实现移植（88% 覆盖），17 号 DDL + 23 号设计随行，不新写；资源缓存盘仅 face=task 分组挂载 |
...
- **THMP 18/23/24 号**（MMagiX 仓 procurement/documents）：TokenHubAdapter 契约来源 + **任务面移植来源**（任务协议权威 = 18 号 §3.7；face-task 实现 = 23 号 §5/§6 的 THMP 网关面平移）；透传模式 = 24 号图① 的网关视角，契约模式 = 24 号图③ 的泛化。
