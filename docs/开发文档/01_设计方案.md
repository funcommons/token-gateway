...
> **实现状态（2026-09-20）**：上列 `TaskClient` 为 **SPI 预留契约**——接口存在于 `gateway-spi`，**全仓无实现**，能力面亦无 task 寻址；任务面实际形态 = **lotask4j 平台托管**（默认，2026-09-01 决议，见《05_任务面lotask4j托管方案》），上文注释中「默认形态是网关本地状态机」的表述已过时。
...
> **实现注记（issue #38）**：`moderation.enabled` 语义自 #38 起兑现——此前版本开关未被管线消费，配了 `moderation.url` 即逐请求无条件发审核 RPC（fail-open 兜底）；#38 起 `enabled=false`（缺省）在 HttpModerationApi 层双闸短路（scan/audit 均不发 RPC），配了 url 但未显式 `enabled: true` 的既有部署升级后审核将停止（启动期 CapabilityValidator WARN 提示）。
...
| **委托面** | TASK_CREATE/TASK_POLL（task 委托面；**SPI 预留、当前无实现**） | 后端自持（后端本身是任务平台） | 后端自带任务状态机，网关只代理与计费 |
...
- **本仓文档族**：用户文档（`04_LLM面接入手册`、`05_任务面接入手册`（规划中 M2.5）、`08_LLM面API契约`、`09_任务面API契约`（规划中 M2.5））与 开发文档（`01_设计方案`、`02_后端接入开发手册`、`03_能力面接口契约`）——与本文 §4/§5 冻结契约互为引用。
