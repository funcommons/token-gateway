...
| 配套 | 设计方案 `01_设计方案.md` §6.4/§7；任务面接入手册 `../用户文档/05_任务面接入手册.md`；安全契约 `04_后端服务对接安全契约方案.md` |
...
| 备选形态 | 私有化/无平台场景保留自有 DB 委托面备选（TaskClient SPI 契约已冻结但**预留未实现**——当前全仓无实现，切换需先立项实现；是适配器决策非架构分叉）。**retask4j 评估结论**：Redis 队列框架，无持久任务表/查询/管理面，状态托管不适用；仅作私有化轻量组合（自有 DB + retask4j 分发）的执行层备选 |
